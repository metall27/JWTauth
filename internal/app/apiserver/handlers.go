...
import (
	...
	"github.com/metall27/JWTauth/internal/app/middleware"
	"github.com/metall27/JWTauth/internal/app/models"
)
...
func (api *APIServer) PostToauth(writer http.ResponseWriter, req *http.Request) {
	...
	//Обрабатываем случай если json вовсе не json
	if err != nil {
		api.logger.Info("Invalid json recieved from client")
		msg := Message{
			StatusCode: 400,
	...
	// Необходимо попытаться обнаружить такого пользователя с такимм логин/пароль в БД
	...
	if err != nil {
		...
		msg := Message{
			StatusCode: 500,
			Message:    "We have troubles while accessing database",
			IsError:    true,
		}
		writer.WriteHeader(500)
		json.NewEncoder(writer).Encode(msg)
	...
	// Надо удостовериться что пароль пользователя в бд совпадает с паролем присланным в json
	// Если пользователь с таким логином есть в бд - проверим что у него код совпадает с фактическим
	...
	token := jwt.New(jwt.SigningMethodHS256)             // Тот же метод подписания что и в JwtMiddleware.go
	...
	//В случае, если токен успешно выбит - отдаём его клиенту
	msg := Message{
		StatusCode: 201,
		Message:    tokenString,
		IsError:    false,
